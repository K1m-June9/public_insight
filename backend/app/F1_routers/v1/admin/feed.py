...
@router.post("/manual", response_model=FeedCreateResponse, status_code=202)
async def create_manual_feeds(
    tasks: BackgroundTasks, 
    feeds: List[FeedManualCreateRequest],
    admin_service: FeedAdminService = Depends(get_admin_feed_service),
    current_user: User = Depends(verify_active_user)
):
    """
    자동 요약 없이, 수동으로 여러 피드 등록
    """
    return await admin_service.create_manual_feeds(feeds)
